...
    // NOVO: Atualiza o nome do monstro e a cor ao carregar o AFK
...
// NOVO: Função para atualizar a cor e o nome do monstro
function updateMonsterVisuals() {
    if (!monsterImage || !monsterNameDisplay || !currentAfkStage) return;

    monsterNameDisplay.textContent = `Goblin Nível ${currentAfkStage}`;

    // Calcula a matiz baseada no estágio
    // A matiz vai de 0 a 360. Podemos mapear os 100 estágios para uma rotação de matiz.
    const hueRotation = (currentAfkStage - 1) * (360 / MAX_AFK_STAGE);
...
    });
    ...
    // NOVO: Mostra o container do monstro
    monsterDisplayContainer.style.display = 'flex';
...
    // const monsterName = `Monstro do Estágio ${currentAfkStage}`; // Não é mais necessário aqui, pois o nome é atualizado no updateMonsterVisuals
...
    updateMonsterHealthDisplay();
    updateMonsterVisuals(); // Garante que o nome e a cor do monstro estejam corretos

    appendCombatLog(`Um Goblin Nível ${currentAfkStage} apareceu! Prepare-se para o combate!`);
}
...
    window.showDamagePopup(damageDealt, isCritical); // Exibe o popup de dano
    ...
    // NOVO: Adiciona a classe para a animação de piscar
    monsterImage.classList.add('flash-animation');
    // Remove a classe após a animação para que possa ser reativada no próximo ataque
...
    combatLog.style.display = 'none';
    // NOVO: Esconde o container do monstro
...
                afkStageSpan.textContent = currentAfkStage; // Atualiza o display do estágio
                updateMonsterVisuals(); // NOVO: Atualiza a cor e o nome do monstro para o novo estágio
