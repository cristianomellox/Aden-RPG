...
// ═══════════════════════════════════════════════════════════════════════
// CAPITAL — SKYBOX 360° (Three.js)
// Mesma engine do Vale Arcano: uma câmera perspectiva dentro de uma esfera
// com a imagem panorâmica (equiretangular) projetada por dentro. Arrastar
// gira a câmera (yaw/pitch); "zoom" estreita/alarga o FOV. Os .city-hotspot
// continuam sendo elementos DOM comuns (mesmo HTML/CSS de antes — nome +
// feixe de luz) — a única coisa nova é que left/top/scale de cada um são
// recalculados a cada frame, projetando sua coordenada esférica (yaw/pitch)
// para pixels de tela. NENHUMA lógica de loja (mercador.js/oficina.js/
// chancelaria.js/mestre de poções) foi tocada.
//
// IMPORTANTE — CALIBRAÇÃO DOS SPOTS:
// As coordenadas yaw/pitch abaixo em CAPITAL_SPOTS são estimativas iniciais
// (calculadas a partir da posição aproximada de cada prédio na imagem
// panorâmica). Elas quase certamente vão precisar de ajuste fino — para
// isso, abra a página com ?debugSpots=1 na URL (exatamente como no Vale
// Arcano): aparecem os contornos tracejados de cada hotspot, e clicando em
// qualquer ponto do céu a tela mostra o yaw/pitch daquele ponto. Copie os
// números para cá.
// ═══════════════════════════════════════════════════════════════════════

const MAP_IMAGE_URL = 'https://aden-rpg.pages.dev/assets/capital.png'; // mesmo nome de arquivo — só troque o PNG no repositório

// Água animada (flow map) do rio/fosso ao redor da capital: máscara (branco =
// onde a água corre, preto = resto do skybox) e flow map (direção do fluxo
// codificada em RG). Mesma técnica da Queda Fontana — ver createWaterFlowMaterial().
// Gere os dois PNGs na ferramenta de pintura e suba com esses nomes exatos.
const WATER_MASK_URL = 'https://aden-rpg.pages.dev/assets/water_mask_capital.png';
const FLOW_MAP_URL   = 'https://aden-rpg.pages.dev/assets/flow_map_capital.png';
...
// yaw/pitch estimados a partir da posição de cada prédio na imagem —
// AJUSTE com ?debugSpots=1 (ver comentário acima).
const CAPITAL_SPOTS = [
    { id: 'btnMercador',      yaw: -40, pitch: -12, width: 200, height: 200 },
...
// ── Material de água animada (flow map) para o skybox da capital ────────
// Mesma técnica usada na Queda Fontana: mostra a textura normal onde o mask
// é preto; onde é branco, distorce a UV de amostragem seguindo a direção
// gravada no flow map (RG, decodificado de 0..1 pra -1..1), com crossfade
// de duas fases pra não "estourar" o offset acumulado.
...
function initSkybox() {
    ...
    texLoader.load(
        ...
        undefined,
        (err) => console.error('[Capital] Falha ao carregar a imagem 360 do mapa:', err)
    );

    // Mask e flow map do rio: só ativa o efeito de água quando os dois
    // carregarem; se algum falhar, o skybox continua normal (estático).
    ...
    texLoader.load(
        ...
        undefined,
        (err) => console.error('[Capital] Falha ao carregar o water mask:', err)
    );
    texLoader.load(
        FLOW_MAP_URL,
        (tex) => { tex.wrapS = tex.wrapT = THREE.ClampToEdgeWrapping; flowTex = tex; tryEnableWater(); },
        undefined,
        (err) => console.error('[Capital] Falha ao carregar o flow map:', err)
    );
    ...
    (function loop(ts) {
        material.uniforms.time.value = ts * 0.001; // anima o flow map da água (rio/fosso da capital)
        if (_sky.pfx) {
            try {
...
function registerAllSpots() {
    const map = document.getElementById('map');
    if (!map) return;
    for (const spot of CAPITAL_SPOTS) {
...
// Idêntica à do Vale Arcano: mostra contorno tracejado nos hotspots e, ao
// clicar em qualquer ponto do céu, mostra o yaw/pitch daquele ponto — cole
// os números em CAPITAL_SPOTS lá em cima.
...
function findSpotConfig(id) { return CAPITAL_SPOTS.find(s => s.id === id); }
...
const PM_SCENE_IMAGE_URL = 'https://aden-rpg.pages.dev/assets/gmdp.png'; // mesma imagem do antigo background do modal da loja
const PM_NPC_IMAGE_URL   = 'https://aden-rpg.pages.dev/assets/mdp_seller_capital.webp';
const PM_TUTORIAL_KEY    = 'mdpTutorialSeen_capital'; // localStorage — tutorial não repete depois da 1ª vez
