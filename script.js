...
// Certifique-se de que a UI está atualizada com o estado correto da sessão
// Imediatamente após a definição do onAuthStateChange
(async () => {
    try {
        // Tenta obter a sessão atual de forma síncrona/rápida
        const { data: { session } } = await supabaseClient.auth.getSession();

        // Se a sessão for null (usuário deslogado), force a exibição da tela de login
        if (!session) {
            console.log("⚠️ Sessão não encontrada. Forçando exibição da tela de login.");
            window.updateUIVisibility(false);
        }
    } catch (e) {
        console.error("Erro ao verificar sessão inicial:", e);
        // Em caso de erro, por segurança, mostra o login para permitir que o usuário tente novamente
        window.updateUIVisibility(false); 
    }
})();
